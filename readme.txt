 ...
 * *type* - post/page/custom post type - if unspecified, default is "page"

 * *name or id* - get the post/page/custom post type by name/slug or ID - if unspecified, default is current post

 * *field* - get a predefined or custom field - if unspecified, default is the main content

You can display any custom field you create, as well as predefined fields: title, id, author, date, url, image, image-url, thumbnail, excerpt.
...
1. A bicycle for sale is added as a new post, with featured image and other info fields.

1. For the content, we create a basic template to display the information:

    <div class="item-wrap">
      Model: [custom field="model"]
      <div class="image-wrap">[custom field="image"]</div>
